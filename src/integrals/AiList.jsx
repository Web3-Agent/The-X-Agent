...
const AiList = () => {
  ...
  const [selectedMemeDesc, setSelectedMemeDesc] = useState(null);
  ...
  const handleConfirmAction = async () => {
    ...
    if (actionType === "Buy") {
      await buyTransactionFunc(selectedMeme?.tokenAddressBase, Web3.utils.toWei(amount, 'ether').toString());
    }
  ...
  const handleMemeClick = (meme) => {
    const memeData = {
      agentName: meme.agentName,
      tokenSupply: meme.tokenSupply,
      agentPrompt: meme.agentPrompt,
      tokenName: meme.tokenName,
    };
    setSelectedMemeDesc(memeData);
  };
  ...
  return (
    <div className="w-full h-full bg-black p-4 flex flex-col items-center space-y-4 overflow-y-auto scrollbar-thin">
      ...
      {selectedMemeDesc ? (
        <AiDetail meme={selectedMemeDesc} onClose={() => setSelectedMemeDesc(null)} />
      ) : (
        AiData.map((meme, index) => (
          <div
            key={index}
          ...
          >
            ...
            <div className="flex flex-col">
              ...
              {/* <p className="text-sm text-gray-400">{meme.tokenName}</p> */}
              <p className="text-xs text-gray-500">
                {meme.agentPrompt.substring(0, 25)}...
              </p>
        ...
        <div className="fixed inset-0 flex items-center justify-center z-50">
          ...
          <div className="bg-[#111111] rounded-lg p-8 z-10 max-w-md w-full border-2 border-gray-300 relative shadow-lg">
            ...
            <input
              ...
              className="bg-input-color border border-transparent focus:border-[#1D9BF0] focus:outline-none focus:bg-bg-color w-full text-white rounded px-3 py-2 w-full mb-4"
            />
            <button
